...
* Create a separate Test class for each day in the following scenario.
	* As a farmer, Froilan must stick to a rigorous agricultural routine.
	...
	## Custom days
  * On `Wednesday` , `Froilan` will `plant` again, 5 crops across <strong>all</strong> crop rows.
  * On `Thursday`, `Froilanda` once again will `fly` over the `Field` with `CropDuster` to fertilize it.
